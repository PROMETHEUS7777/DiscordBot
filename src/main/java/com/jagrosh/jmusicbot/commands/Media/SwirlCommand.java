 ...
 */
public class SwirlCommand extends MediaCommand
{
    
    
    public SwirlCommand(Bot bot)
    {
        
        this.name = "swirl";
        this.help = "swirls an image or gif";
        this.aliases = bot.getConfig().getAliases(this.name);
        this.arguments = "<image/gif>";

		this.children = new SlashCommand[]{new SwirlCommand.fromURL(), new SwirlCommand.fromFile()};
    }
	...
	private static class fromURL extends SlashCommand {
		public fromURL() {
			this.name = "url";
			this.help = "swirl an image/gif from a url";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.STRING, "url", "the url of the image/gif to swirl").setRequired(true));
			...
			event.deferReply().queue(
					hook -> {
						...
						try {
							...
							hook.editOriginal("Failed to swirl").queue();
							e2.printStackTrace();
							return;
						}
						...
						try {
							for (int i = 0; i < iarray.length; i++) {
								iarray[i] = iarray[i].swirlImage(90);
							}
						} catch (MagickException e) {
							hook.editOriginal("Failed to swirl").queue();
							e.printStackTrace();
						...
						hook.editOriginalAttachments(FileUpload.fromData(blob, "swirled_" + filename)).queue();

						//finalize processing message
						hook.editOriginal("it's dizzy now").queue();
					}
			);
		}
	}
	...
	private static class fromFile extends SlashCommand {
		public fromFile() {
			this.name = "file";
			this.help = "swirl an image/gif from a file";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.ATTACHMENT, "file", "the image/gif to swirl").setRequired(true));
    ...
    protected void execute(CommandEvent event)
    {
    	...
    	try {
			...
			event.replyError("Failed to swirl");
			e2.printStackTrace();
			return;
		}
    	...
    	//send finished image/gif
    	event.getChannel().sendFiles(FileUpload.fromData(blob, "swirled_" + filename)).queue();
    	
    	//finalize processing message
    	event.getChannel().editMessageById(pmsg.getId(),"it's dizzy now").queue();
