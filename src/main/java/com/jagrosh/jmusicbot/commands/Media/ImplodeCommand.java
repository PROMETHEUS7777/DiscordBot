...
public class ImplodeCommand extends MediaCommand
{
    ...
    public ImplodeCommand(Bot bot)
    {
		...
		this.children = new SlashCommand[]{new ImplodeCommand.fromURL(), new ImplodeCommand.fromFile()};
    }
	...
	private static class fromURL extends SlashCommand {
		public fromURL() {
			this.name = "url";
			this.help = "implode an image/gif from a url";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.STRING, "url", "the url of the image/gif to implode").setRequired(true));
			...
			event.deferReply().queue(
					hook -> {
						...
						try {
							...
							hook.editOriginal("Failed to implode").queue();
							e2.printStackTrace();
							return;
						}
						...
						hook.editOriginalAttachments(FileUpload.fromData(blob, "imploded_" + filename)).queue();

						//finalize processing message
						hook.editOriginal("We do a little imploding").queue();
					}
			);
		}
	}
	...
	private static class fromFile extends SlashCommand {
		public fromFile() {
			this.name = "file";
			this.help = "implode an image/gif from a file";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.ATTACHMENT, "file", "the image/gif to implode").setRequired(true));
    ...
    protected void execute(CommandEvent event)
    {
    	...
    	//send finished image/gif
    	event.getChannel().sendFiles(FileUpload.fromData(blob, "imploded_" + filename)).queue();
