 ...
 */
public class FastGifCommand extends MediaCommand
{
    
    
    public FastGifCommand(Bot bot)
    {
        
        this.name = "fast";
        this.help = "speed up a gif";
        this.aliases = bot.getConfig().getAliases(this.name);
        this.arguments = "<gif>";

		this.children = new SlashCommand[]{new FastGifCommand.fromURL(), new FastGifCommand.fromFile()};
    }
	...
	private static class fromURL extends SlashCommand {
		public fromURL() {
			this.name = "url";
			this.help = "speed up a gif from a url";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.STRING, "url", "the url of the gif to speed up").setRequired(true));
			...
			event.deferReply().queue(
					hook -> {
						...
						try(InputStream iStream = url.openStream()){
							...
							hook.editOriginal("Unable to download gif, please try again").queue();
							return;
						}
						...
						try {
							...
							hook.editOriginal("Failed to fast").queue();
							e2.printStackTrace();
							return;
						}
						...
						//speed image
						try {
							if (iarray[0].getDelay() == 2) {
								iarray = ImageUtil.removeOtherElements(iarray);
							}
							else {
								for (int i = 0; i < iarray.length; i++) {
									...
									iarray[i].setDelay((int) (iarray[i].getDelay() * 0.5));
						...
						hook.editOriginalAttachments(FileUpload.fromData(blob, "fast_" + filename)).queue();

						//finalize processing message
						hook.editOriginal("gif is speed").queue();
					}
			);
		}
	}
	...
	private static class fromFile extends SlashCommand {
		public fromFile() {
			this.name = "file";
			this.help = "speed up a gif from a file";

			List<OptionData> options = new ArrayList<>();
			options.add(new OptionData(OptionType.ATTACHMENT, "file", "the gif to speed up").setRequired(true));
    ...
    protected void execute(CommandEvent event)
    {
    	...
    	try {
			...
			event.replyError("Failed to fast");
			e2.printStackTrace();
			return;
		}
    	...
    	//send finished image/gif
    	event.getChannel().sendFiles(FileUpload.fromData(blob, "fast_" + filename)).queue();
    	
    	//finalize processing message
    	event.getChannel().editMessageById(pmsg.getId(),"gif is speed").queue();
