...
public class RollCommand extends DiceCommand
{
    ...
    public RollCommand(Bot bot)
    {
        ...
        options.add(new OptionData(OptionType.STRING, "toroll", "the dice to roll, also supports bonuses. ex: 2d10+8").setRequired(true));
    ...
    protected void execute(SlashCommandEvent event)
    {
        ...
        String toRoll = event.getOption("roll").getAsString();
        if (toRoll.isEmpty()) {
            toRoll = "1d20";
        }

        //make toRoll all lowercase
        ...
        for (int it = 1; it < toRoll.length(); it++)
        {
            switch (toRoll.charAt(it))
            {
                case 'd':
                    ...
                    {
                        event.reply("You dont need to roll that many of them").queue();
                        return;
                    }
                    lit = it;
                    break;
                ...
                case'-':
                    switch (toRoll.charAt(lit))
                    {
                        case 'd':
                            ...
                            {
                                event.reply("Your dice aren't that big").queue();
                                return;
                            }
                            break;
                        case '+':
                        case'-':
                            ...
                            {
                                event.reply("Your bonus isn't that long").queue();
                                return;
                            }
                            break;
                    }
        ...
        if (rolls.isEmpty())
        {
            event.reply("Invalid roll command. Please try again").queue();
        }
